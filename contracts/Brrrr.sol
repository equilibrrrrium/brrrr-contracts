...
contract Brrrr is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 12h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 75000 ether;
    // Distribution for DAO wallet
    uint256 public constant INITIAL_DAO_WALLET_DISTRIBUTION = 5000 ether;
    ...
    /**
     * @notice Constructs the BRRRR ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("EquiliBRRRRium.money", "BRRRR") {
        // Mints 1 BRRRR to contract creator for initial pool setup
    ...
    function _getBrrrrPrice() internal view returns (uint256 _brrrrPrice) {
        try IOracle(brrrrOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Brrrr: failed to fetch BRRRR price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _brrrrPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_brrrrPrice >= taxTiersTwaps[tierId]) {
    ...
    function setBrrrrOracle(address _brrrrOracle) public onlyOperatorOrTaxOffice {
        require(_brrrrOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints Brrrr to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of Brrrr to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentBrrrrPrice = _getBrrrrPrice();
            currentTaxRate = _updateTaxRate(currentBrrrrPrice);
            if (currentBrrrrPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        for(uint256 i = 0; i < _genesisPools.length; i++) {
            ...
            _mint(_genesisPools[0], 52500); // 52500 for BNB/BRRRR genesis
            _mint(_genesisPools[i], 11250); // 11250 for BNB genesis
            _mint(_genesisPools[i], 11250); // 11250 for BUSD genesis
        }
